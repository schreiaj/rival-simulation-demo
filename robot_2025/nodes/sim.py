...
class SimulationNode(BaseNode):
    ...
    def _startSimulation(self):
        try:
            ...
            p.configureDebugVisualizer(p.COV_ENABLE_GUI, 0)  # Disable the GUI overlay for better performance
            p.configureDebugVisualizer(p.COV_ENABLE_SHADOWS, 0)  # Disable shadows for better performance
            p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 1)  # Keep rendering enabled
            ...
            self._joint_indices = {}
            for i in range(p.getNumJoints(self._robot_id, physicsClientId=self._physics_client)):
                joint_info = p.getJointInfo(self._robot_id, i, physicsClientId=self._physics_client)
            ...
            # Verify robot state
            pos, orn = p.getBasePositionAndOrientation(self._robot_id, physicsClientId=self._state_client)
    ...
    def step(self):
        try:
            ...
            if self._joints_updated and self._joint_indices_array:
                print(f"Applying joint states to {len(self._joint_indices_array)} joints")
                self._joints_updated = False
                
                # Due to a bug in pybullet, if you care about joint velocity caps you need to set them individually
